...
gulp.task('build-app', function () {
    ...
    var bApp = config.env === 'dev' ? watchify(browserify(browserifyOptions)) : browserify(browserifyOptions);

    bApp.transform('babelify', { presets: ['es2015'] })
        .transform('glslify');
    ...
    if(config.env === 'dev') {
        ...
        bApp.on('time', function (time) { gutil.log('-> watchify on time', time); });
        bApp.on('bytes', function (bytes) { gutil.log('-> watchify on bytes', bytes); });
...
gulp.task('prod', function(callback) {
    runSequence(['build-vendor', 'build-app', 'less'], 'modernizr' );
});
